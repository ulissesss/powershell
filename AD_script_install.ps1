...
Rename-Computer -NewName valerio -Force -Passthru
...
$users=Import-Csv -Path "c:\script\users.csv" | Select -ExpandProperty first
$groups=Import-Csv -Path "c:\script\group.csv" | Select -ExpandProperty name
$j = 0
